---
description: >
  Conducts deep, multi-source research on a question — clarifies via
  AskUserQuestion, consults prior tix research/plans, spawns parallel
  web-search-researcher subagents, cross-verifies claims, and produces a cited
  report saved to tix/research/. Use when the user asks to research,
...
`tix/research/`. If no topic was provided, ask for it before Phase 1.

---

## Phase 1: Clarify (mandatory — do not skip)

Before searching anything, ask the user these questions. Use the `AskUserQuestion`
tool to ask all of them in one batched interaction (a single call containing
three questions). If `AskUserQuestion` is unavailable, fall back to asking
