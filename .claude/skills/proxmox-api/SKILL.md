...
Known hosts and passwords for this user's boxes are in Claude **memory**
(look for a `proxmox-*` memory), not in this skill. `root` logs in as
`root@pam` (the `@pam` realm is required).
